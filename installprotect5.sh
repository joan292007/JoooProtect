...
<div class="jhonaley-footer">
  <div class="jt-inner">
    ...
    <span class="jt-text">Panel by <a href="https://t.me/atentrei" target="_blank">Jooo Protect</a></span>
...
for LF in "${LAYOUT_FILES[@]}"; do
  if [ -f "$LF" ]; then
    if grep -q "<title>" "$LF" && ! grep -q "Jooo Protect" "$LF"; then
      sed -i 's/<title>.*<\/title>/<title>Pterodactyl - Jooo Protect<\/title>/g' "$LF" 2>/dev/null
